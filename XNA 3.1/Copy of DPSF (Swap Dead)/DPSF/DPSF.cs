...
namespace DPSF
{
...
#endif
    {
        ...
        private int miLastActiveParticleIndex = -1; // Holds the Index of the Last Active Particle in the Particles Array
...
// Else we are on the Xbox 360
    ...
    // Else we are inheriting DrawableGameComponent
        ...
        public int NumberOfParticlesAllocatedInMemory
        {
            ...
            set
            {
                ...
                if (iNewMaxNumberOfParticles >= 0)
                {
                    ...
                    if (miLastActiveParticleIndex >= 0)
                    {
                        // Save that the Particles will need to be copied over to the new Particle array
                        bThereAreOldParticlesToCopy = true;
                    }

                    // Save a handle to the current Particles array so that it isn't lost right away, in case 
                    // we need to copy Particles over to the new Particles Array
                    ...
                    // If there are Particles to copy to the new Particle Array
                    if (bThereAreOldParticlesToCopy)
                    {
                        // If there were more Active Particles than there is space for in the new Particles Array
                        if (miLastActiveParticleIndex >= iNewMaxNumberOfParticles)
                        {
                            miLastActiveParticleIndex = iNewMaxNumberOfParticles - 1;
                        }

                        // Copy the old Active Particles to the new Particles Array
                        for (int iIndex = 0; iIndex <= miLastActiveParticleIndex; iIndex++)
                        {
                            mcParticles[iIndex] = cOldParticles[iIndex];
                        }
        ...
        public int NumberOfActiveParticles
        {
            get
            {
                ...
                if (IsInitialized())
                {
                    ...
                    return miLastActiveParticleIndex + 1;
                }
        ...
        /// <summary>
        /// Returns the array of all Particle objects. Active Particles are kept at the start of the array. 
        /// Particles in indices less than NumberOfActiveParticles are Active Particles; all indices 
        /// greater than NumberOfActiveParticles are Inactive Particles.
        /// </summary>
        ...
        /// <summary>
        /// Adds a new Particle to the particle system, at the end of the Active Particles segment. 
        /// This new Particle is initialized using the particle system's Particle Initialization Function
        /// </summary>
        /// <returns>True if a particle was added, False if there is not enough memory for another Particle</returns>
        ...
        /// <summary>
        /// Adds a new Particle to the particle system, at the end of the Active Particles segment. Returns true if
        ...
        public bool AddParticle(Particle cParticleToCopy)
        {
            ...
            if (NumberOfActiveParticles >= miMaxNumberOfParticlesAllowed)
            {
            ...
            if (NumberOfActiveParticles >= NumberOfParticlesAllocatedInMemory)
            {
                ...
                if (AutoMemoryManagerSettings.MemoryManagementMode == AutoMemoryManagerModes.IncreaseAndDecrease ||
                    AutoMemoryManagerSettings.MemoryManagementMode == AutoMemoryManagerModes.IncreaseOnly)
                {
                    // Calculate what the new increased Max Number Of Particles Allocated In Memory should be
                    ...
                    // Make sure we do not allocate more than the Max Number Of Particles Allowed
                    iNewMaxNumberOfParticles = (int)MathHelper.Min(iNewMaxNumberOfParticles, MaxNumberOfParticlesAllowed);

                    // Set the new Max Number Of Particles Allocated In Memory
                    NumberOfParticlesAllocatedInMemory = iNewMaxNumberOfParticles;
                }
                // Else we cannot increase the Number Of Particles Allocated In Memory
                else
                {
                    // Return that we cannot add a new Particle at this time
                    return false;
            ...
            // Increase the Number of Active Particles and get a handle to the new Active Particle
            miLastActiveParticleIndex++;
            Particle cParticle = mcParticles[miLastActiveParticleIndex];
        ...
        {
            // Reset the Index of the Last Active Particle
            miLastActiveParticleIndex = -1;
        }
    ...
    // If inheriting from DrawableGameComponent, we need to override the Update() function
        ...
        private void Update(float fElapsedTimeInSeconds, bool bCalledByDrawableGameComponent)
        {
            ...
            if (iNumberOfParticlesToEmit > 0)
            {
                ...
                while (iParticlesEmitted < iNumberOfParticlesToEmit && bCanStillAddParticles)
                {
                    ...
                    if (bCanStillAddParticles)
                    {
                        ...
                        // Else the Particle is no longer Active already
                        else
                        {
                            // Remove the Particle from the Active Particles segment
                            miLastActiveParticleIndex--;
                        }
                    }
                }
            ...
            // Process all of the Active Particles, except for any Particles that were just added to the
            // Particle System since they've already been updated.
            // Process them in reverse order so that the newest particles are Drawn last.
            int iIndex = 0;
            for (iIndex = (miLastActiveParticleIndex - iNumberOfNewParticlesAdded); iIndex >= 0; iIndex--)
            {
                // Get a handle to the Particle
                Particle cParticle = mcParticles[iIndex];
                ...
                // Else the Particle is no longer Active
                else
                {
                    // Swap this Particle with the last Active one
                    Particle cTemp = mcParticles[miLastActiveParticleIndex];
                    mcParticles[miLastActiveParticleIndex] = mcParticles[iIndex];
                    mcParticles[iIndex] = cTemp;

                    // Decrement the Last Active Particle Index
                    miLastActiveParticleIndex--;
                }
            }
