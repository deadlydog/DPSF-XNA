...
namespace DPSF.ParticleSystems
{
    ...
    class AnimatedQuadParticleSystem : DefaultAnimatedTexturedQuadParticleSystem
    {
        ...
        public void LoadEvents()
        {
            ...
            InitialProperties.PositionMin = new Vector3(-100, 0, -100);
        ...
        public void KeepParticleInBox(DefaultAnimatedTexturedQuadParticle cParticle, float fElapsedTimeInSeconds)
        {
            int iBoxX = 100;
            int iBoxY = 100;
            int iBoxZ = 100;
            ...
            if ((cParticle.Position.X > iBoxX && cParticle.Velocity.X > 0) ||
                (cParticle.Position.X < -iBoxX && cParticle.Velocity.X < 0))
            {
            ...
            // If the Particle is heading outside of the box
            if ((cParticle.Position.Y > iBoxY && cParticle.Velocity.Y > 0) ||
                (cParticle.Position.Y < 0 && cParticle.Velocity.Y < 0))
            {
                // Reverse it's velocity to keep it inside the box
            ...
            // If the Particle is heading outside of the box
            if ((cParticle.Position.Z > iBoxZ && cParticle.Velocity.Z > 0) ||
                (cParticle.Position.Z < -iBoxZ && cParticle.Velocity.Z < 0))
            {
                // Reverse it's velocity to keep it inside the box
        ...
        public void ChangeDirectionRandomly(DefaultAnimatedTexturedQuadParticle cParticle, float fElapsedTimeInSeconds)
        {
