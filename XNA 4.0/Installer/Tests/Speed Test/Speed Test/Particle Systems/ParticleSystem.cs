...
namespace DPSF.ParticleSystems
{
	...
	/// </summary>
	class ParticleSystem : DefaultTexturedQuadParticleSystem
	{
		/// <summary>
		/// Constructor
		/// </summary>
		...
		public void InitializeParticleProperties(DefaultTexturedQuadParticle cParticle)
		{
			...
			cParticle.Orientation = Orientation3D.Rotate(Matrix.CreateRotationZ((float)RandomNumber.NextDouble() * MathHelper.TwoPi), cParticle.Orientation);
			cParticle.RotationalVelocity.Z = -MathHelper.TwoPi + (float)(RandomNumber.NextDouble() * (2 * MathHelper.TwoPi));
		}
		...
		public void UpdateParticleVelocity(DefaultTexturedQuadParticle cParticle, float fElapsedTimeInSeconds)
		{
			cParticle.Velocity = DPSFHelper.RandomVectorBetweenTwoVectors(msVelocityMin, msVelocityMax);
		}
